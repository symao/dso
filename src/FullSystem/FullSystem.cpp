...
Vec4 FullSystem::trackNewCoarse(FrameHessian* fh) {
    ...
    if (allFrameHistory.size() == 2)
        ...
        // if they don't work they will only be tried on the coarsest level, which is super fast
        // anyway. also, if tracking rails here we loose, so we really, really want to avoid that.
    ...
    for (unsigned int i = 0; i < lastF_2_fh_tries.size(); i++) {
        ...
        if (i != 0) {
            printf(
                "RE-TRACK ATTEMPT %d with initOption %d and start-lvl %d (ab %f %f): %f %f %f %f "
                "%f -> %f %f %f %f %f \n",
                i, i, pyrLevelsUsed - 1, aff_g2l_this.a, aff_g2l_this.b, achievedRes[0],
                achievedRes[1], achievedRes[2], achievedRes[3], achievedRes[4],
                coarseTracker->lastResiduals[0], coarseTracker->lastResiduals[1],
                coarseTracker->lastResiduals[2], coarseTracker->lastResiduals[3],
        ...
        if (haveOneGood) {
            for (int i = 0; i < 5; i++) {
                if (!std::isfinite((float)achievedRes[i]) ||
                    achievedRes[i] > coarseTracker->lastResiduals[i])  // take over if achievedRes
    ...
    if (!haveOneGood) {
        printf(
            "BIG ERROR! tracking failed entirely. Take predictred pose and hope we may somehow "
            "recover.\n");
        flowVecs = Vec3(0, 0, 0);
...
void FullSystem::traceNewCoarse(FrameHessian* fh) {
    ...
    //	printf("ADD: TRACE: %'d points. %'d (%.0f%%) good. %'d (%.0f%%) skip. %'d (%.0f%%) badcond.
    //%'d (%.0f%%) oob. %'d (%.0f%%) out. %'d (%.0f%%) uninit.\n", 			trace_total, 			trace_good,
...
void FullSystem::activatePointsMT_Reductor(std::vector<PointHessian*>* optimized,
                                           std::vector<ImmaturePoint*>* toOptimize, int min,
...
void FullSystem::activatePointsMT() {
    ...
    for (FrameHessian* host : frameHessians)  // go through all active frames
    {
        ...
        for (unsigned int i = 0; i < host->immaturePoints.size(); i += 1) {
            ...
            // delete points that have never been traced successfully, or that are outlier on the
            // last trace.
            ...
            bool canActivate =
                (ph->lastTraceStatus == IPS_GOOD || ph->lastTraceStatus == IPS_SKIPPED ||
            ...
            Vec3f ptp =
                KRKi * Vec3f(ph->u, ph->v, 1) + Kt * (0.5f * (ph->idepth_max + ph->idepth_min));
    ...
    //			(int)toOptimize.size(), immature_deleted, immature_notReady, immature_needMarg,
...
void FullSystem::flagPointsForRemoval() {
    ...
    {
        for (int i = ((int)frameHessians.size()) - 1; i >= 0 && i >= ((int)frameHessians.size());
             i--)
...
void FullSystem::addActiveFrame(ImageAndExposure* image, int id, const SE3& prior_pose) {
    if (isLost)
        return;
    boost::unique_lock<boost::mutex> lock(trackMutex);
    ...
    shell->camToWorld = prior_pose;  // no lock required, as fh is not used anywhere yet.
...
void FullSystem::makeKeyFrame(FrameHessian* fh) {
...

    {
        ...
        coarseTracker_forNewKF->debugPlotIDepthMap(&minIdJetVisTracker, &maxIdJetVisTracker,
                                                   outputWrapper);
    ...
    ef->dropPointsF();
    getNullspaces(ef->lastNullspaces_pose, ef->lastNullspaces_scale, ef->lastNullspaces_affA,
...
void FullSystem::initializeFromInitializer(FrameHessian* newFrame) {
    ...
    // int numPointsTotal = makePixelStatus(firstFrame->dI, selectionMap, wG[0], hG[0],
    // setting_desiredDensity); int numPointsTotal = pixelSelector->makeMaps(firstFrame->dIp,
...
void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth) {
    ...
    // int numPointsTotal = makePixelStatus(newFrame->dI, selectionMap, wG[0], hG[0],
    // setting_desiredDensity);
    int numPointsTotal =
        pixelSelector->makeMaps(newFrame, selectionMap, setting_desiredImmatureDensity);
...
void FullSystem::printLogLine() {
    ...
    if (!setting_debugout_runquiet)
        printf(
            "LOG %d: %.3f fine. Res: %d A, %d L, %d M; (%'d / %'d) forceDrop. a=%f, b=%f. Window "
            "%d (%d)\n",
            allKeyFramesHistory.back()->id, statistics_lastFineTrackRMSE, ef->resInA, ef->resInL,
            ef->resInM, (int)statistics_numForceDroppedResFwd,
            (int)statistics_numForceDroppedResBwd, allKeyFramesHistory.back()->aff_g2l.a,
    ...
    if (numsLog != 0) {
        (*numsLog) << allKeyFramesHistory.back()->id << " " << statistics_lastFineTrackRMSE << " "
                   ...
                   << (int)statistics_lastNumOptIts << " " << ef->resInA << " " << ef->resInL << " "
