...
ImmaturePointStatus ImmaturePoint::traceOn(FrameHessian* frame, const Mat33f& hostToFrame_KRKi,
    ...
    if (debugPrint)
        printf("trace pt (%.1f %.1f) from frame %d to %d. Range %f -> %f. t %f %f %f!\n", u, v,
               host->shell->id, frame->shell->id, idepth_min, idepth_max, hostToFrame_Kt[0],
    ...
    //	const float extraSlackOnTH = 1.2;			// for energy-based outlier check, be slightly more
    //relaxed by this factor.
    //	const float slackInterval = 0.8;			// if pixel-interval is smaller than this, leave it
    //be. 	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave
    //it be.
    ...
    // ============== compute error-bounds on result in pixel. if the new interval is not at least
    // 1/2 of the old, SKIP ===================
    ...
    if (debugPrint)
        printf(
            "trace pt (%.1f %.1f) from frame %d to %d. Range %f (%.1f %.1f) -> %f (%.1f %.1f)! "
            "ErrorInPixel %.1f!\n",
            u, v, host->shell->id, frame->shell->id, idepth_min, uMin, vMin, idepth_max, uMax, vMax,
            errorInPixel);
    ...
    for (int i = 0; i < numSteps; i++) {
        ...
        for (int idx = 0; idx < patternNum; idx++) {
            float hitColor =
                getInterpolatedElement31(frame->dI, (float)(ptx + rotatetPattern[idx][0]),
    ...
    for (int it = 0; it < setting_trace_GNIterations; it++) {
        ...
        for (int idx = 0; idx < patternNum; idx++) {
            Vec3f hitColor =
                getInterpolatedElement33(frame->dI, (float)(bestU + rotatetPattern[idx][0]),
            ...
            float residual =
                hitColor[0] - (hostToFrame_affine[0] * color[idx] + hostToFrame_affine[1]);
        ...
        if (energy > bestEnergy) {
            ...
            if (debugPrint)
                printf("GN BACK %d: E %f, H %f, b %f. id-step %f. UV %f %f -> %f %f.\n", it, energy,
                       H, b, stepBack, uBak, vBak, bestU, bestV);
        } else {
            gnStepsGood++;
            ...
            if (debugPrint)
                printf("GN step %d: E %f, H %f, b %f. id-step %f. UV %f %f -> %f %f.\n", it, energy,
                       H, b, step, uBak, vBak, bestU, bestV);
        }
    ...
    //	float absGrad1 = getInterpolatedElement(frame->absSquaredGrad[1],bestU*0.5-0.25,
    //bestV*0.5-0.25, wG[1]); 	float absGrad2 =
    //getInterpolatedElement(frame->absSquaredGrad[2],bestU*0.25-0.375, bestV*0.25-0.375, wG[2]);
...
double ImmaturePoint::linearizeResidual(CalibHessian* HCalib, const float outlierTHSlack,
    ...
    for (int idx = 0; idx < patternNum; idx++) {
        ...
        if (!projectPoint(this->u, this->v, idepth, dx, dy, HCalib, PRE_RTll, PRE_tTll, drescale, u,
