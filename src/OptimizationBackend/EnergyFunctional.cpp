...
void EnergyFunctional::resubstituteF_MT(VecX x, CalibHessian* HCalib, bool MT) {
  ...
  if (MT)
    red->reduce(std::bind(&EnergyFunctional::resubstituteFPt, this, cstep, xAd, std::placeholders::_1,
                          std::placeholders::_2, std::placeholders::_3, std::placeholders::_4),
                0, allPoints.size(), 50);
  else
...
double EnergyFunctional::calcLEnergyF_MT() {
  ...
  red->reduce(std::bind(&EnergyFunctional::calcLEnergyPt, this, std::placeholders::_1, std::placeholders::_2,
